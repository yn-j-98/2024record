...
public class CommunityPageAction implements Action{
    ...
    public ActionForward execute(HttpServletRequest request, HttpServletResponse response) {
        ...
        String path = "Community.jsp"; // 전체 글 페이지로 이동
        ...
        // 글정보와 키워드를 요청에서 가져옴
        String condition = request.getParameter("board_list"); // 글 정보
        ...
        if(condition != null) {    
            ...
            if(condition.equals("SEARCH_ID")) {
                ...
                boardDTO.setBoard_searchKeyword(keyword); // 검색 내용 설정
                ...
                boardCount.setBoard_searchKeyword(keyword);//검색내용
                boardCount = boardDAO.selectOne(boardCount);//
                listNum = boardCount.getBoard_total();//게시글의 전체 개수
                System.out.println("전체 페이지 개수 (아이디 검색)"+listNum);//로그
            }
                ...
                System.out.println("전체 페이지 개수 (이름 검색)"+listNum);//로그
            }
                ...
                boardCount = boardDAO.selectOne(boardCount);
                listNum = boardCount.getBoard_total();
                System.out.println("전체 페이지 개수 (제목 검색)"+listNum);//로그
            }
