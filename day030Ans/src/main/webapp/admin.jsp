...
<form action="controller.jsp?action=INSERTPRODUCT" method="POST">
	<input type="hidden" name="seller" value="<%=session.getAttribute("member")%>">
...
### 상품 목록 ###
	...
	<%
		for(ProductDTO data:(ArrayList<ProductDTO>)request.getAttribute("datas")){
			out.println("<li><a href='controller.jsp?action=DELETEPRODUCT&num="+data.getNum()+"'>["+data.getName()+"]</a></li>");
		}
	%>
	</ol>
<hr>
