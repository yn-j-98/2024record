...
//        프로필foreach하고 set으로 주소값추가
        ...
        for(BattleDTO data : model_battle_datas) {
        	data.setModel_battle_gym_profile(request.getServletContext().getContextPath() + "/profile_img/" + filename);
        }
        ...
        filename = memberDTO.getModel_member_crew_profile();        
        //model_member_rank_datas에 담아서 보내기
        for(MemberDTO data : model_crew_rank_datas) {
        	data.setModel_member_crew_profile(request.getServletContext().getContextPath() + "/profile_img/" + filename);
        }
        ...
        filename = datas.getModel_member_profile();
        for(MemberDTO data : model_member_rank_datas) {//프로필이 여러개여서 어레이리스트로
        	data.setModel_member_profile(request.getServletContext().getContextPath() + "/profile_img/" + filename);
        }
